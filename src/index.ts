...
db.prepare(
  "CREATE TABLE IF NOT EXISTS codes (code_id INTEGER, code TEXT, message_id, requester TEXT, assignee TEXT, added_at TEXT, resolved_at TEXT, status TEXT, uncommon INTEGER, rare INTEGER, epic INTEGER, legendary INTEGER, guardian TEXT, PRIMARY KEY (code_id AUTOINCREMENT), FOREIGN KEY (requester) REFERENCES users(user_id), FOREIGN KEY (assignee) REFERENCES users(user_id))"
).run();
