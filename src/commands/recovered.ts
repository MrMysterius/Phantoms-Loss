...
export async function recovered(message: Discord.Message, user: userData) {
  ...
  verify_embed.setDescription(
    "Please paste the code ingame for the verification. Then check if the person who completed it is listed below. If so then use the command `verify <code_id> yes`, if not please type `verify <code_id> no`."
  );
